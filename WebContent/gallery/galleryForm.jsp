...
<script type="text/javascript">
	function fileValid() {
		...
		// let isValid = true;
		// button 타입은 submit으로 하고 form 태그의 onsubmit 속성에 유효성 검사 함수를 설정할 때는 isValid 변수는 필요 없습니다.
		// console.log(document.forms[0].pic.value)
		if(imgfile.value == "" || imgfile.value == null) {
			alert('파일첨부는 필수 입니다.')
			// isValid = false;
			return false;		// insubmit 속성으로 호출된 함수에서 return false이면 submit 안합니다.
		}else {
			...
			if(imgfile.files[0].size > maxSize) {
				// imgfile.value 는 선택한 파일명, imgfile.files[0] 다중 선택 가능하므로 파일 자체를 인덱스로 지정합니다. 
...
<body>
	...
	<!-- 
		...
		2. 서버 페이지에서는 file 타입 요소로 전달된 파일을 다른 파라미터(입력값)과 함께 전송받습니다.(업로드)
			ㄴ 서버에서 업로드 폴더 설정이 필요 (예시 : C:\0524_java_LBG\upload)
